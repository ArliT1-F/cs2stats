      ...
      <div className="space-y-3 lg:col-span-2">
        {top3.map((w, i) => (
          <div key={w.name} className="relative overflow-hidden border border-cs-border bg-cs-panel p-4 clip-corner">
            ...
            <div className="font-mono text-[10px] uppercase tracking-widest text-cs-orange">// {i === 0 ? "PRIMARY" : i === 1 ? "SECONDARY" : "TERTIARY"}</div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {top.map((w) => {
                ...
                return (
                  <tr key={w.name} className="border-b border-cs-border/50">
                    <td className="py-2 font-display font-bold text-white">{w.name}</td>
